...
DIM trigger_duration, delay, reset_time AS FLOAT
...
init:
  ...
  count_time = (Par_3)/10
  sleep_duration = sequence_duration + 122 - (2*count_time) 
  ...
  delay = (21252 + 10) * 3 ' * 10 since the variables are in 10 ns and /(10/3) as the value has to be in number or clock ticks which is 10/3 for T11 processor
  Processdelay = delay
  i = 0
  DO
...
event:
  ...
  DO 
    ...
    IO_SLEEP(trigger_duration)
    Data_1[j] = Data_1[j] + Cnt_Read(1)        ' accumulate sig
    DIGOUT(16, 0)
    IO_Sleep(reset_time)
    DIGOUT(16, 1)
    Cnt_Clear(1)           ' Clear counter
    IO_Sleep(trigger_duration)
    Data_2[j] = Data_2[j] + Cnt_Read(1)        ' accumulate ref
    DIGOUT(16, 0)
    j = j + 1
  UNTIL (j = 20)
  Cnt_Enable(0)
