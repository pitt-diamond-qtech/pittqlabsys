...
from src.core.read_write_functions import save_aq_file, load_aq_file
...
class Script(QObject):
    ...
    def __init__(self, name=None, settings=None, devices=None, scripts=None, log_function=None, data_path=None):
        ...
        Args:
            ...
            devices (optional): devices used in the script
    ...
    def _SCRIPTS(self):
        ...
        Returns: a dictionary of the devices, where the key is the device name and value is the device class
        if there is not device it should return an empty dict
    ...
    def instrumets(self):
        """
        :return: devices that the script uses as a dictionary
        """
    ...
    def scripts(self, script_dict):
        ...
        assert set(script_dict.keys()) == set(self._SCRIPTS.keys()), "{:s}: set subscripts {:s}, received {:s}".format(
            self.name, str(list(script_dict.keys())), str(list(self._SCRIPTS.keys())))
    ...
    def settings(self):
        ...
        settings contain Parameters, devices and Scripts
        '''
    ...
    def run(self):
        ...
        self.log('starting script {:s} at {:s} on {:s}'.format(self.name, self.start_time.strftime('%H:%M:%S'),
        ...
        self.log('script {:s} finished at {:s} on {:s}'.format(self.name, self.end_time.strftime('%H:%M:%S'),
    ...
    def to_dict(self):
        ...
        from AQuISS.src.core.script_iterator import ScriptIterator

        if 'script_iterator' in self.__module__.split('.'):
            # script iterator module is of the form
            # 'AQuISS.src.core.script_iterator.dynamic_script_iterator0'
    ...
    def save_aq(self, filename=None):
        ...
        #     if len(filename.split('\\\\?\\')) == 1:
        #         filename = '\\\\?\\' + filename
        save_aq_file(filename, scripts=self.to_dict(), overwrite=True)
    ...
    def load(filename, devices=None):
        ...
        Args:
            ...
            devices:
                optional - only needed if script requires devices
                dictionary of form
        ...
        script_instance, _, updated_devices = Script.load_and_append({'script': script_class},
                                                                         devices=devices)
        script_instance = script_instance['script']

        # save references to devices
        devices = script_instance._devices

        # update the script instance
        script_instance.__dict__ = script_as_dict

        # update references to devices
        script_instance._devices = devices

        return script_instance, updated_devices
    ...
    def load_settings(path, setttings_only=True):
        """
        loads the settings that has been save with Script.save_aq.
        Args:
            path: path to folder saved by Script.save_aq
            setttings_only: if true returns only the settings if the .aq file contains only a single script
        Returns:
        ...
        settings = load_aq_file(fname)['scripts']
    ...
    @staticmethod
    def load_and_append(script_dict, scripts=None, devices=None, log_function=None, data_path=None,
                        raise_errors=False, package='AQuISS', verbose=False):
        """
        load script from script_dict and append to scripts, if additional devices are required create them and add them to devices
        ...
        def get_devices(class_of_script, script_devices, devices):
            """

            creates the dictionary with the devices needed for the script and update the device dictionary if new devices are required

            Args:
                class_of_script: the class of the script
                devices: the devices that have been loaded already

            Returns: dictionary with the devices that the script needs and the updated devices dictionary

            """

            default_devices = getattr(class_of_script, '_DEVICES')
            device_dict = {}
            devices_updated = {}
            devices_updated.update(devices)
            # check if devices needed by script already exist, if not create an instance
            for device_name, device_class in default_devices.items():
                # check if devices needed by script already exist
                ...
                if script_devices is not None and device_name in script_devices:
                    device_settings_dict = script_devices[device_name]['settings']
                else:
        ...
        def get_sub_scripts(class_of_script, devices, sub_scripts_dict, log_function=None):
            """

            creates the dictionary with the sub scripts needed by the script and updates the device dictionary if new devices are required
            ...
            #
            # create devices that script needs
            sub_scripts = {}
            sub_scripts, scripts_failed, devices_updated = Script.load_and_append(default_scripts, sub_scripts,
                                                                                      devices,
            ...
            return sub_scripts, devices_updated
        ...
        for script_name, script_info in script_dict.items():
            ...
            if script_name in list(scripts.keys()):
                ...
                module, script_class_name, script_settings, script_devices, script_sub_scripts, script_doc, package = Script.get_script_information(
                    script_info, package=package)
                ...
                if 'ScriptIterator' in script_class_name:
                    ...
                    from src.core.script_iterator import \
                        ScriptIterator  # CAUTION: imports ScriptIterator, which inherits from script. Local scope should avoid circular imports.
                ...
                #  ========= create the devices that are needed by the script =========
                try:
                    script_devices, updated_devices = get_devices(class_of_script, script_devices,
                                                                              updated_devices)
                except Exception as err:
                    print(('loading script {:s} failed. Could not load devices!'.format(script_name)))
                ...
                try:
                    sub_scripts, updated_devices = get_sub_scripts(class_of_script, updated_devices,
                                                                       script_sub_scripts, log_function=log_function)
                except Exception as err:
                ...
                #  ========= create the script if devices and subscripts have been loaded successfully =========
                class_creation_string = ''
                if script_devices:
                    class_creation_string += ', devices = script_devices'
                if sub_scripts:
        ...
        return updated_scripts, load_failed, updated_devices

    @staticmethod
    def get_script_information(script_information, package='AQuISS', verbose=False):
        """
        ...
        Args:
            ...
            package (optional): name of the package to which the script belongs, i.e. AQuISS .
                                Only used when script_information is a string
        Returns:
            module, script_class_name, script_settings, script_devices, script_sub_scripts, script_info, package
        """
        ...
        if isinstance(script_information, dict):
            ...
            if 'package' in script_information:
                ...
                assert 'filepath' in script_information  # there should be a filepath if we load form a aq file
            ...
            if 'devices' in script_information:
                script_devices = script_information['devices']
            ...
            # to avoid this problem call from AQuISS.src.core import Script (otherwise the path to Script is __main__.Script)
        ...
        # if the script has not been created yet, i.e. script_class_name: ScriptIteratorAQ or ScriptIterator
        if verbose:
        ...
        if len(script_class_name.split('dynamic_script_iterator')) == 2 and \
            ...
            # package = 'AQuISS' # all the dynamic iterator scripts are defined in the name space of AQuISS
            # all the dynamic iterator scripts are defined in the name space of package.AQuISS.src.core.script_iterator
            # module = import_module(package + '.AQuISS.src.core.script_iterator')
            module_path = package
        ...
        if module is None or not hasattr(module, script_class_name):
            ...
            raise ImportError(
                'Could not find the module that contains ' + script_class_name + ' in module ' + module_path)

        # if the module has a name of type dynamic_script_iteratorX where X is a number the module is script iterator
        return module, script_class_name, script_settings, script_devices, script_sub_scripts, script_info, package

    @staticmethod
    def get_script_module(script_information, package='AQuISS', verbose=False):
        """
        ...
        Args:
            script_information: information of the script. This can be
                ...
                - a Script instance
                - name of Script class
            package (optional): name of the package to which the script belongs, i.e. AQuISS  only used when script_information is a string
        Returns:
            module

        """

        module, _, _, _, _, _, _ = Script.get_script_information(script_information=script_information, package=package,
                                                                 verbose=verbose)
